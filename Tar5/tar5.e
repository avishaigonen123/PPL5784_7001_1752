...
for i=1 to length(only_jack_filenames) do 
	...
	integer fd_file_output_parsing_xml = open(path & "\\" & only_jack_filenames[i] & "_our.xml","w") 
	integer fd_file_output_parsing_vm = open(path & "\\" & only_jack_filenames[i] & ".vm","w") 
	startClass()
    parsing(fd_file_input_parsing, fd_file_output_parsing_xml, fd_file_output_parsing_vm)

	close(fd_file_input_parsing)
	close(fd_file_output_parsing_xml)
	close(fd_file_output_parsing_vm)
	close(fd_output)
end for
